...
function CreateTokenModal({ visible, onClose, onSuccess, creatorWallet, activeWallet }: CreateTokenModalProps) {
  ...
  const handleCreate = async () => {
    // Prevent concurrent launches — each call generates a new mintKeypair
    ...
    try {
      ...
      const rawSignAndSend = launchpadSigningService.makeSignAndSend(signer);
      // Wrap to persist signature in sessionStorage so confirmation can resume on refresh
      signAndSend = async (tx, signers) => {
        const sig = await rawSignAndSend(tx, signers);
        try { sessionStorage.setItem('dawen_pending_launch_sig', sig); } catch {}
        return sig;
      };
    } catch (sigErr: any) {
    ...
    try {
      ...
      if (res.success && res.mintAddress && res.txSignature) {
        try { sessionStorage.removeItem('dawen_pending_launch_sig'); } catch {}
        ...
        <KeyboardAvoidingView style={mStyles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={0}>
          ...
          {step === 'form' && (
            <ScrollView style={mStyles.flex} contentContainerStyle={mStyles.formContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <TouchableOpacity onPress={handleCreate} style={[mStyles.launchBtn, step === 'progress' && { opacity: 0.5 }]} disabled={step === 'progress'}>
