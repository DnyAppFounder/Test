...
export default function ProfileScreen() {
  ...
  const [crewRoles, setCrewRoles] = useState<Array<{ role_key: string; role_name: string; badge_color: string; badge_icon: string }>>([]);
  ...
  const loadProfile = useCallback(async () => {
    ...
    // Load crew roles for this profile
    if (profileData?.id) {
      const { data: memberRows } = await supabase
        .from('crew_members')
        .select('role_key, crew_roles(role_name, badge_color, badge_icon)')
        .eq('user_id', profileData.id)
        .in('status', ['active', 'trial']);
      if (memberRows) {
        setCrewRoles(memberRows.map((m: any) => ({
          role_key: m.role_key,
          role_name: m.crew_roles?.role_name ?? m.role_key,
          badge_color: m.crew_roles?.badge_color ?? '#8B5CF6',
          badge_icon: m.crew_roles?.badge_icon ?? 'shield',
        })));
      }
    }
      ...
      >
            ...
            {profile.twitter_url ? (
              ...
              >
                <Svg width={15} height={15} viewBox="0 0 24 24" fill="none">
                  <Path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.402 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622 5.91-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z" fill="#E7E9EA" />
                </Svg>
                <Text style={[styles.socialLinkText, { color: '#E7E9EA' }]}>X</Text>
              </TouchableOpacity>
            ) : null}
        ...
        {crewRoles.length > 0 && (
          <View style={styles.crewBadgesRow}>
            {crewRoles.map(r => (
              <View key={r.role_key} style={[styles.crewBadge, { backgroundColor: r.badge_color + '22', borderColor: r.badge_color + '55' }]}>
                <Text style={[styles.crewBadgeText, { color: r.badge_color }]}>{r.role_name}</Text>
              </View>
            ))}
          </View>
        )}
            ...
            {verifyLoading ? (
              ...
              <>
                ...
                <Text style={styles.verifyDesc}>
                  Complete all 4 steps below to automatically receive your blue verification badge.
                </Text>
                ...
                <View style={styles.verifySteps}>
                  ...
                  {/* Step 3 */}
                  <View style={styles.verifyStep}>
                    <View style={[styles.verifyStepIcon, verifyStatus?.followsDawenPulse && styles.verifyStepDone]}>
                      {verifyStatus?.followsDawenPulse
                        ? <Check size={14} color="#fff" strokeWidth={3} />
                        : <Text style={styles.verifyStepNum}>3</Text>
                      }
                    </View>
                    <View style={styles.verifyStepInfo}>
                      <Text style={styles.verifyStepTitle}>Follow @DawenPulse</Text>
                      <Text style={styles.verifyStepSub}>Follow the official DawenPulse account</Text>
                    </View>
                    {verifyStatus?.dawenPulseId && (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          setShowVerifyModal(false);
                          router.push(`/profile/${verifyStatus!.dawenPulseId}` as any);
                        }}
                        activeOpacity={0.8}
                      >
                        <Text style={styles.verifyActionBtnText}>
                          {verifyStatus?.followsDawenPulse ? 'Done' : 'Follow'}
                        </Text>
                      </TouchableOpacity>
                    )}
                  </View>
                  ...
                  {/* Step 4 */}
                  <View style={styles.verifyStep}>
                    <View style={[styles.verifyStepIcon, verifyStatus?.sentBlueDM && styles.verifyStepDone]}>
                      {verifyStatus?.sentBlueDM
                        ? <Check size={14} color="#fff" strokeWidth={3} />
                        : <Text style={styles.verifyStepNum}>4</Text>
                      }
                    </View>
                    <View style={styles.verifyStepInfo}>
                      <Text style={styles.verifyStepTitle}>DM "Blue" to @BlueBadge</Text>
                      <Text style={styles.verifyStepSub}>Send the message "Blue" to @BlueBadge</Text>
                    </View>
                    {verifyStatus?.blueBadgeId && (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          setShowVerifyModal(false);
                          router.push(`/chat/${verifyStatus!.blueBadgeId}` as any);
                        }}
                        activeOpacity={0.8}
                      >
                        <Text style={styles.verifyActionBtnText}>
                          {verifyStatus?.sentBlueDM ? 'Done' : 'DM'}
                        </Text>
                      </TouchableOpacity>
                    )}
                  </View>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    !(verifyStatus?.followsDecent && verifyStatus?.followsBadge && verifyStatus?.followsDawenPulse && verifyStatus?.sentBlueDM) && styles.saveBtnDisabled,
                  ]}
...
const styles = StyleSheet.create({
  ...
  bannerWrap: { width: '100%', height: 200, backgroundColor: '#1A0B2E', overflow: 'hidden' },
  bannerEditOverlay: {
    position: 'absolute',
    bottom: 8,
    right: 12,
  ...
  profileTopRow: {
    ...
    paddingHorizontal: spacing.lg, marginTop: -48, marginBottom: spacing.lg, gap: spacing.md,
  },
  avatarWrap: {
    borderRadius: 52, borderWidth: 4, borderColor: '#0A0A0F',
    overflow: 'visible',
  ...
  profileTitleBlock: { flex: 1, paddingBottom: 4, gap: 2 },
  nameVerifiedRow: { flexDirection: 'row', alignItems: 'center', gap: 6, flexWrap: 'wrap' },
  displayName: { fontSize: 22, fontWeight: '900', color: colors.textPrimary, letterSpacing: -0.5 },
  handle: { fontSize: fontSize.md, fontWeight: '500', color: colors.textMuted },
  shareBtn: {
    width: 38, height: 38, borderRadius: 19, backgroundColor: '#1A1A28',
    justifyContent: 'center', alignItems: 'center',
  ...
  socialLinkBtn: {
    ...
    backgroundColor: '#16161F', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)',
  },
  socialLinkText: { fontSize: 13, fontWeight: '700' },
  crewBadgesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, paddingHorizontal: spacing.lg, marginBottom: spacing.lg },
  crewBadge: {
  ...
  statsRow: {
    ...
    backgroundColor: '#12121A', borderRadius: 18, paddingVertical: 16,
  ...
  statClickable: { color: colors.textPrimary },
  statLabel: { fontSize: 11, color: colors.textMuted, marginTop: 3, fontWeight: '600', letterSpacing: 0.3 },
  ...
  actionBtn: {
    flex: 2, backgroundColor: colors.primary, borderRadius: borderRadius.full,
    paddingVertical: 14, alignItems: 'center', justifyContent: 'center',
    flexDirection: 'row', gap: spacing.xs,
  },
  actionBtnText: { fontSize: fontSize.md, fontWeight: '700', color: colors.white },
  followingBtn: { backgroundColor: 'rgba(139,92,246,0.15)', borderWidth: 1.5, borderColor: colors.primary },
  followingBtnText: { color: colors.primary },
  messageBtn: {
    flex: 1, backgroundColor: '#16161F', borderRadius: borderRadius.full,
  ...
  tabBar: {
    ...
    borderBottomColor: 'rgba(255,255,255,0.06)', marginBottom: spacing.xs,
  },
  tab: { flex: 1, paddingVertical: 14, alignItems: 'center', position: 'relative' },
  tabText: { fontSize: 13, fontWeight: '700', color: colors.textMuted, letterSpacing: 0.3 },
  tabTextActive: { color: colors.primary },
  tabUnderline: { position: 'absolute', bottom: 0, left: '15%', right: '15%', height: 2.5, backgroundColor: colors.primary, borderRadius: 2 },
