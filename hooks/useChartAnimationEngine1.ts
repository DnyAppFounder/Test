...
export interface ChartAnimationActions {
   ...
   * deltaMs > 0 → drag right → move into past (offset increases).
   * deltaMs < 0 → drag left → move toward live (offset decreases).
   */
...
export function useChartAnimationEngine(
  ...
  const setInitialPanOffsetMs = useCallback((ms: number) => {
    ...
    const clamped = Math.max(0, Math.min(maxPanBackMsRef.current, ms));
  ...
  return {
    ...
    actions: { onPanStart, onPanDelta, onPanEnd, returnToLive, setChartVisible, setInitialPanOffsetMs },
  };
}
