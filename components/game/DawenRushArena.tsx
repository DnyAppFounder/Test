...
import {
  View, Text, StyleSheet, TouchableOpacity, PanResponder, Platform,
} from 'react-native';
...
export function DawenRushArena({ seed, mode, entryId, matchId, entryAmountSol, onGameEnd }: Props) {
  ...
  // ── Arena ─────────────────────────────────────────────────────────────────
  return (
    <View style={[styles.arenaWrap, { height: arenaH + 8 }]}
      ...
      <View style={[styles.arena, { height: arenaH }]}>
        ...
        {Array.from({ length: 6 }, (_, i) => (
          <View key={`vg${i}`} style={[styles.gridV, { left: ((i + 1) / 7) * containerW }]} />
        ))}
        ...
        >
          <View style={styles.joystickHint}>
            <Text style={styles.joystickHintText}>
              {Platform.OS === 'web' ? 'WASD / Arrows' : 'Drag to move'}
            </Text>
          </View>
        </View>
      </View>
