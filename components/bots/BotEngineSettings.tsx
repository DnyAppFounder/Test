...
import { Bot, Plus, Trash2, ChevronRight, X, Check, Shield, Bell, Star, Globe, Sword, Trophy, Heart, Settings, FileText, RefreshCw, TriangleAlert as AlertTriangle, ExternalLink } from 'lucide-react-native';
...
interface BotModule { module_name: string; is_enabled: boolean; config: Record<string, unknown> }
interface BotCommand { id: string; command: string; description: string; response_text: string; is_builtin: boolean; is_enabled: boolean; cooldown_seconds: number; allowed_roles: string[] }
interface DawenBot { id: string; bot_type: BotType; bot_name: string; bot_avatar_url: string | null; command_prefix: string; is_enabled: boolean; modules: BotModule[]; commands: BotCommand[] }
interface BotLog { id: string; action_type: string; command: string | null; details: any; created_at: string; actor: { username: string; avatar_url: string | null } | null }
interface RaidTask { id: string; title: string; target_url: string; required_actions: string[]; participant_count: number; status: string; reward_points: number; ends_at: string | null }
...
const BOT_TYPE_META: Record<BotType, { label: string; desc: string; color: string; icon: any }> = {
  core:     { label: 'DAWEN Core Bot',     desc: 'All modules in one bot',                color: '#06b6d4', icon: Bot },
  guard:    { label: 'DAWEN Guard Bot',    desc: 'Anti-spam, captcha, link filter',       color: '#f59e0b', icon: Shield },
  sentinel: { label: 'DAWEN Sentinel Bot', desc: 'Warnings, mute, kick, ban',             color: '#ef4444', icon: AlertTriangle },
  welcome:  { label: 'DAWEN Welcome Bot',  desc: 'Greetings, rules, community links',     color: '#10b981', icon: Heart },
  pulse:    { label: 'DAWEN Pulse Bot',    desc: 'Announcements and post publishing',     color: '#3b82f6', icon: Bell },
  oracle:   { label: 'DAWEN Oracle Bot',   desc: 'Token info, price, chart data',         color: '#8b5cf6', icon: Globe },
  raid:     { label: 'DAWEN Raid Bot',     desc: 'X/Twitter raids and participation',     color: '#f97316', icon: Sword },
  reward:   { label: 'DAWEN Reward Bot',   desc: '$DAWORLD points, rank, referrals',      color: '#eab308', icon: Trophy },
};
...
export default function BotEngineSettings({ groupId, walletAddress, isAdmin, visible, onClose }: Props) {
  ...
  useEffect(() => {
    if (visible) { loadBots(); setView('list'); }
  }, [visible, loadBots]);
  ...
  const handleSaveBotName = async () => {
    ...
    try {
      ...
      if (res.success) {
        await loadBots();
        setSelectedBot(prev => prev ? { ...prev, bot_name: editName, command_prefix: editPrefix } : prev);
      }
    } finally {
      setSaving(false);
    }
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}>
            <Text style={styles.statNum}>{botCount}</Text>
            <Text style={styles.statLabel}>Total Bots</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={[styles.statNum, { color: colors.success }]}>{activeBots}</Text>
          ...
          <TouchableOpacity style={styles.statCard} onPress={handleOpenRaids}>
            <Sword size={20} color={colors.textSecondary} />
        ...
        {loading ? (
          ...
          bots.map(bot => {
            const meta = BOT_TYPE_META[bot.bot_type] ?? BOT_TYPE_META.core;
            ...
            return (
              <TouchableOpacity key={bot.id} style={styles.botCard} onPress={() => handleOpenDetail(bot)}>
                ...
                <View style={styles.botInfo}>
                  ...
                  <Text style={styles.botPrefix}>Prefix: <Text style={{ color: meta.color }}>{bot.command_prefix}</Text></Text>
                </View>
                {isAdmin && (
                  <Switch
  ...
  function renderAdd() {
    return (
      <ScrollView showsVerticalScrollIndicator={false}>
        <Text style={styles.sectionTitle}>Choose Bot Type</Text>
        {(Object.keys(BOT_TYPE_META) as BotType[]).map(type => {
          const meta = BOT_TYPE_META[type];
          const IconComp = meta.icon;
          const isSelected = addBotType === type;
          const alreadyAdded = bots.some(b => b.bot_type === type);
          return (
            <TouchableOpacity
              key={type}
              style={[styles.botTypeCard, isSelected && { borderColor: meta.color }]}
              onPress={() => !alreadyAdded && setAddBotType(type)}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.botName, alreadyAdded && { color: colors.textMuted }]}>{meta.label}</Text>
                <Text style={styles.botDesc}>{meta.desc}</Text>
              </View>
              {alreadyAdded
                ? <Text style={{ color: colors.textMuted, fontSize: fontSize.xs }}>Added</Text>
        ...
        <TouchableOpacity
          ...
          disabled={saving}
        >
          {saving ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.primaryBtnText}>Add {BOT_TYPE_META[addBotType].label}</Text>}
        </TouchableOpacity>
  ...
  function renderDetail() {
    ...
    const meta = BOT_TYPE_META[selectedBot.bot_type] ?? BOT_TYPE_META.core;
    const IconComp = meta.icon;
    ...
    return (
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.detailHeader, { borderColor: `${meta.color}30` }]}>
          ...
          <View style={{ flex: 1 }}>
            <View style={styles.botNameRow}>
              <Text style={styles.detailBotName}>{selectedBot.bot_name}</Text>
              <View style={styles.botBadge}><Text style={styles.botBadgeText}>BOT</Text></View>
            </View>
        ...
        {/* Modules */}
        {selectedBot.modules.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Modules</Text>
            {selectedBot.modules.map(mod => {
              const modMeta = MODULE_META[mod.module_name] ?? { label: mod.module_name, color: colors.textSecondary };
              return (
                <TouchableOpacity key={mod.module_name} style={styles.moduleCard} onPress={() => isAdmin && handleOpenModule(mod)}>
                  <View style={[styles.moduleColorDot, { backgroundColor: modMeta.color }]} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.moduleName}>{modMeta.label}</Text>
                    <Text style={styles.moduleStatus}>{mod.is_enabled ? 'Enabled' : 'Disabled'}</Text>
                  </View>
        ...
        <View style={styles.section}>
          ...
          {isAdmin && (
            <TouchableOpacity
              ...
              onPress={() => handleDeleteBot(selectedBot)}
            >
              <Trash2 size={16} color={colors.error} />
              <Text style={[styles.actionText, { color: colors.error }]}>Delete Bot</Text>
            </TouchableOpacity>
          )}
        </View>
  ...
  function renderModule() {
    ...
    const modMeta = MODULE_META[selectedModule.module_name] ?? { label: selectedModule.module_name, color: colors.textSecondary };
    ...
    return (
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={[styles.detailHeader, { borderColor: `${modMeta.color}30` }]}>
          <View style={[styles.moduleColorDot, { width: 36, height: 36, borderRadius: 18, backgroundColor: `${modMeta.color}20` }]} />
          ...
          raids.map(raid => (
            <View key={raid.id} style={styles.raidCard}>
              <View style={styles.raidHeader}>
                <Text style={styles.raidTitle}>{raid.title}</Text>
  ...
  function renderCreateRaid() {
    ...
    return (
      <ScrollView showsVerticalScrollIndicator={false}>
        <Text style={styles.sectionTitle}>New Raid Task</Text>
        ...
        <Text style={styles.fieldLabel}>Target URL (X post / profile)</Text>
        ...
        <TextInput style={[styles.input, { height: 72, textAlignVertical: 'top' }]} value={raidDesc} onChangeText={setRaidDesc} placeholder="What to do, context..." placeholderTextColor={colors.textMuted} multiline />
        ...
        <TextInput style={[styles.input, { width: 100 }]} value={raidPoints} onChangeText={setRaidPoints} keyboardType="numeric" placeholderTextColor={colors.textMuted} />
  ...
  const VIEW_TITLES: Record<ScreenView, string> = {
    ...
    module: selectedModule ? (MODULE_META[selectedModule.module_name]?.label ?? selectedModule.module_name) : 'Module',
    logs: 'Bot Logs',
    raids: 'Raid Tasks',
...
const MODULE_META: Record<string, { label: string; color: string }> = {
  guard:    { label: 'Guard',    color: '#f59e0b' },
  sentinel: { label: 'Sentinel', color: '#ef4444' },
  welcome:  { label: 'Welcome',  color: '#10b981' },
  pulse:    { label: 'Pulse',    color: '#3b82f6' },
  oracle:   { label: 'Oracle',   color: '#8b5cf6' },
  raid:     { label: 'Raid',     color: '#f97316' },
  reward:   { label: 'Reward',   color: '#eab308' },
};
...
function raidStatusColor(status: string): string {
  ...
  if (status === 'ended') return colors.textMuted;
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: colors.textPrimary, fontSize: fontSize.md, fontWeight: '600' },
  ...
  botIconBox: { width: 40, height: 40, borderRadius: borderRadius.sm, alignItems: 'center', justifyContent: 'center' },
  botInfo: { flex: 1 },
  botNameRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginBottom: 2 },
  botName: { color: colors.textPrimary, fontSize: fontSize.sm, fontWeight: '600' },
  botBadge: {
    backgroundColor: colors.primary + '30', borderRadius: 4, paddingHorizontal: 4, paddingVertical: 1,
  },
  botBadgeText: { color: colors.primaryLight, fontSize: 9, fontWeight: '700', letterSpacing: 0.5 },
  ...
  detailIconLarge: { width: 48, height: 48, borderRadius: borderRadius.sm, alignItems: 'center', justifyContent: 'center' },
  detailBotName: { color: colors.textPrimary, fontSize: fontSize.md, fontWeight: '700' },

  section: { marginBottom: spacing.xl },
  sectionTitle: { color: colors.textSecondary, fontSize: fontSize.xs, fontWeight: '600', letterSpacing: 1, textTransform: 'uppercase', marginBottom: spacing.sm },
  ...
  moduleColorDot: { width: 10, height: 10, borderRadius: 5 },
  ...
  cmdRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginBottom: spacing.xs },
  cmdName: { color: colors.primary, fontSize: fontSize.sm, fontWeight: '600', minWidth: 90 },
  ...
  configLabel: { color: colors.textSecondary, fontSize: fontSize.sm },
  configFieldWrap: { marginBottom: spacing.sm },
  ...
  logTypeBadge: { borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2, minWidth: 70, alignItems: 'center' },
  ...
  actionChipActive: { borderColor: colors.primary, backgroundColor: colors.primaryMuted },
