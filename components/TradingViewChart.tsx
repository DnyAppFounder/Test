...
export function TradingViewChart({
  ...
  // ms-based pan delta captured at the moment a gesture starts (delta-from-start math).
  ...
  // Local visibility mirror for the WS reconnect guard (also kept in sync with engine).
  ...
  // Geometry refs — written during render, read inside stable gesture closures.
  ...
  // panOffsetCandles is derived from the engine for all downstream consumers.
  const panOffsetCandles = bucketMs > 0 ? animEngine.state.panOffsetMs / bucketMs : 0;
  const rightTime        = animEngine.state.visualRightTime - animEngine.state.panOffsetMs;
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_e, gestureState) => {
        ...
        // Dragging right (positive dx) = scrolling into the past (offset increases).
        // Dragging left (negative dx)  = moving toward live (offset decreases).
        ...
        const clampedMs = animEngine.state.panOffsetMs;
        // Compute delta from current engine state so engine clamping stays correct.
        animEngine.actions.onPanDelta(newOffsetMs - clampedMs);
      },
