  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Migration: onboarding_complete from legacy 'onboarding_completed' global key.
        // Old code wrote 'onboarding_completed'; per-wallet key is 'onboarding_complete'.
        let finalOnboardingComplete = onboardingCompleteVal;
        if (!finalOnboardingComplete && addr) {
          const legacyVal = await AsyncStorage.getItem('onboarding_completed').catch(() => null);
          if (legacyVal === 'true') {
            finalOnboardingComplete = 'true';
            writeKeyWalletOnly(addr, 'onboarding_complete', 'true').catch(() => {});
            console.log(`[Security] Migrated legacy onboarding_complete for ${addr.slice(0, 8)}`);
          }
        }

        // Migration helper for step-completion flags that may only exist in legacy global keys.
        const migrateFlagIfMissing = async (
          current: string | null,
          suffix: string,
          globalKey: string,
        ): Promise<string | null> => {
          if (current !== null || !addr) return current;
          const legacy = await AsyncStorage.getItem(globalKey).catch(() => null);
          if (legacy !== null) writeKeyWalletOnly(addr, suffix, legacy).catch(() => {});
          return legacy;
        };

        const [
          finalBiometricEnabled,
          finalBiometricOffered,
          finalSeedBackup,
          finalImportBackup,
          finalExternalWarning,
        ] = await Promise.all([
          migrateFlagIfMissing(biometricEnabledVal,  'biometric_enabled',         G.biometricEnabled),
          migrateFlagIfMissing(biometricOfferedVal,  'biometric_offered',         G.biometricOffered),
          migrateFlagIfMissing(seedBackupVal,        'seed_backup_confirmed',     G.seedBackupConfirmed),
          migrateFlagIfMissing(importBackupVal,      'import_backup_confirmed',   G.importBackupConfirmed),
          migrateFlagIfMissing(externalWarningVal,   'external_warning_accepted', G.externalWarningAccepted),
        ]);

        // Migration: pin_hash — check legacy global key, then Supabase backup.
        let finalPinHash = pinHashVal;
        if (!finalPinHash && addr) {
          const legacyPin = await AsyncStorage.getItem(G.pinHash).catch(() => null);
          if (legacyPin) {
            finalPinHash = legacyPin;
            writeKeyWalletOnly(addr, 'pin_hash', legacyPin).catch(() => {});
            console.log(`[Security] Migrated legacy pin_hash for ${addr.slice(0, 8)}`);
          } else {
            try {
              ...
              if (user) {
                ...
                if (data?.pin_hash) {
                  ...
                  writeKeyWalletOnly(addr, 'pin_hash', data.pin_hash).catch(() => {});
                }
              }
            } catch {}
          }
        ...
        console.log(`[Security] Loaded for ${addr.slice(0, 8)}: pin=${!!finalPinHash} type=${walletTypeVal} onboarding=${finalOnboardingComplete} seed=${finalSeedBackup} import=${finalImportBackup}`);

        if (!cancelled) {
          setState({
            pinHash:                  finalPinHash,
            walletType:               (walletTypeVal as WalletSecurityType) ?? null,
            biometricEnabled:         finalBiometricEnabled === 'true',
            biometricOffered:         finalBiometricOffered === 'true',
            seedBackupConfirmed:      finalSeedBackup === 'true',
            importBackupConfirmed:    finalImportBackup === 'true',
            externalWarningAccepted:  finalExternalWarning === 'true',
            onboardingComplete:       finalOnboardingComplete === 'true',
            isLoaded: true,
            loadedForAddr: addr,
          });
        }
