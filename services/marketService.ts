...
export class MarketService {
  static async getTopCoins(): Promise<MarketCoin[]> {
    try {
      const res = await fetch(`${FUNCTION_URL}?action=top`, {
        headers,
      ...
      if (isTimeout) console.warn(`[MarketService] getTopCoins timeout (8s) @ ${Date.now()}`);
      return [];
    }
  }
  ...
  static async searchCoins(query: string): Promise<MarketCoin[]> {
    try {
      ...
      const isTimeout = e?.name === 'TimeoutError' || e?.name === 'AbortError';
      if (isTimeout) console.warn(`[MarketService] searchCoins timeout (8s) @ ${Date.now()}`);
      return [];
    }
  }
  ...
  static async getCoinDetail(id: string): Promise<CoinDetail | null> {
    try {
      ...
      const isTimeout = e?.name === 'TimeoutError' || e?.name === 'AbortError';
      if (isTimeout) console.warn(`[MarketService] getCoinDetail timeout (8s) @ ${Date.now()}`);
      return null;
    }
  }
