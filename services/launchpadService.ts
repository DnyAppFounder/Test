...
class LaunchpadService {
  ...
  async uploadImage(walletAddress: string, imageUri: string): Promise<string | null> {
    try {
      ...
      const filename = `launches/${walletAddress}/${Date.now()}/image.${ext}`;
  ...
  async uploadMetadata(metadata: object, tokenId: string, walletAddress?: string): Promise<string> {
    const jsonStr = JSON.stringify(metadata);
    const blob = new Blob([jsonStr], { type: 'application/json' });
    const folder = walletAddress ? `launches/${walletAddress}/${tokenId}` : `launches/${tokenId}`;
